...
float getWind(vec2 vertex, vec2 uv, float timer){
	...
	float wind = (sin(time) + cos(time * detail)) * strength * max(0.0, (1.0-uv.x) - heightOffset);
	
	return wind;
	}

void vertex() {
	if ((getWind(VERTEX.xy, UV, TIME)<0.0 && direction > 0.0) || (getWind(VERTEX.xy, UV, TIME)>0.0 && direction < 0.0))
	{
		VERTEX.y += -1.0*getWind(VERTEX.xy, UV, TIME);
	}
	else
	{
		VERTEX.y += 1.0*getWind(VERTEX.xy, UV, TIME);
